...
path_1.each do |vector|
  direction = vector.slice(0)
  distance = vector.slice(1..).to_i

  current_x = tracked_path_1.last[0]
  current_y = tracked_path_1.last[1]

  puts "current: #{tracked_path_1.last}"
  ...
  distance.times do |i|
    ...
    case direction
    when UP
      tracked_path_1 << [current_x, current_y += 1]
    when DOWN
      tracked_path_1 << [current_x, current_y -= 1]
    when LEFT
      tracked_path_1 << [current_x -= 1, current_y]
    when RIGHT
      tracked_path_1 << [current_x += 1, current_y]
    end
  end

  puts "ended up at #{tracked_path_1.last}"
  puts
end

# puts tracked_path_1.to_s

# ---------------

tracked_path_2 = [[0, 0]]

path_2.each do |vector|
  # process(tracked_path_2, vector)
  direction = vector.slice(0)
  distance = vector.slice(1..).to_i

  current_x = tracked_path_2.last[0]
  current_y = tracked_path_2.last[1]

  puts "current: #{tracked_path_2.last}"
  puts "#{direction} #{distance}"
  ...
  distance.times do |i|
    ...
    case direction
    when UP
      tracked_path_2 << [current_x, current_y += 1]
    when DOWN
      tracked_path_2 << [current_x, current_y -= 1]
    when LEFT
      tracked_path_2 << [current_x -= 1, current_y]
    when RIGHT
      tracked_path_2 << [current_x += 1, current_y]
    end
  end

  puts "ended up at #{tracked_path_2.last}"
  puts

  if (tracked_path_1.include?(tracked_path_2.last))
    puts "-- match: #{tracked_path_2.last}"
  end
end

# puts tracked_path_2.to_s

intersections = tracked_path_1 & tracked_path_2

foo = intersections.map do |x, y|
  puts "#{x.abs} + #{y.abs} = #{(x.abs + y.abs)}"
